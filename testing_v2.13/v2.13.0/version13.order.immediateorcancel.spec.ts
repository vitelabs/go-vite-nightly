...
describe("test version13 upgrade", () => {
  ...
  it("test FundContract placeOrder - ImmediateOrCancelOrder - fully fill", async () => {
    ...
    await dex.placeOrder(user2FundContract, tradeToken, quoteToken, sideBuy, orderImmediateOrCancel, price, quantity); // buy 6 VTT
    ...
    assert.equal(viteValue1_after, new Big(initViteAmount).add(dex.getSellerObtainAmount(price, quantity, takerPlatformFeeRate, takerBrokerFeeRate, tradeTokenDecimal, quoteTokenDecimal)).toFixed());
    ...
    assert.equal(viteValue2_after, (new Big(initViteAmount).sub(dex.getBuyerCostAmount(price, quantity, takerPlatformFeeRate, takerBrokerFeeRate, tradeTokenDecimal, quoteTokenDecimal))).toFixed());
    assert.equal(viteValueLocked2_after, "0");
    assert.equal(vttValue2_after, new Big(initVttAmount).add(new Big(quantity)).toFixed());
  ...
  it("test FundContract placeOrder - ImmediateOrCancelOrder - partially fill", async () => {
    const initViteAmount = "100000000000000000000000"; // 10w VITE
    const initVttAmount = "100000000000000000000000";  // 10w VTT
    ...
    await dex.placeOrder(user1FundContract, tradeToken, quoteToken, sideSell, orderLimit, price, quantity); // sell 6 VTT
    await dex.placeOrder(user2FundContract, tradeToken, quoteToken, sideBuy, orderImmediateOrCancel, price, quantity1); // buy 8 VTT
  ...
  it("test FundContract placeOrder - ImmediateOrCancelOrder - not fill at all", async () => {
    const initViteAmount = "100000000000000000000000"; // 10w VITE
    const initVttAmount = "100000000000000000000000";  // 10w VTT
    ...
    const quantity = "6000000000000000000"
    await dex.placeOrder(user1FundContract, tradeToken, quoteToken, sideSell, orderLimit, price, quantity); // sell 6 VTT
    await dex.placeOrder(user2FundContract, tradeToken, quoteToken, sideBuy, orderImmediateOrCancel, price1, quantity); // buy 6 VTT
