...
describe("test version13 upgrade", () => {
  ...
  it("test FundContract placeOrder - FillOrKillOrder - not have tx", async () => {
    ...
    await dex.placeOrder(user1FundContract, tradeToken, quoteToken, sideSell, orderFillOrKill, price3, quantity1); // sell 2 VTT
  ...
  it("test FundContract placeOrder - FillOrKillOrder - partial fill", async () => {
    const initViteAmount = "100000000000000000000000"; // 10w VITE
    const initVttAmount = "100000000000000000000000";  // 10w VTT
    ...
    const price2 = "6000"
    const quantity = "6000000000000000000"
    const quantity1 = "8000000000000000000"
    await dex.placeOrder(user1FundContract, tradeToken, quoteToken, sideSell, orderLimit, price1, quantity); // sell 6 VTT
    await dex.placeOrder(user2FundContract, tradeToken, quoteToken, sideBuy, orderLimit, price2, quantity); // buy 6 VTT
    await dex.placeOrder(user1FundContract, tradeToken, quoteToken, sideSell, orderFillOrKill, price2, quantity1); // sell 8 VTT
  ...
  it("test FundContract placeOrder - FillOrKillOrder - fully fill - Sell", async () => {
    const initViteAmount = "100000000000000000000000"; // 10w VITE
    const initVttAmount = "100000000000000000000000";  // 10w VTT
    ...
    await dex.placeOrder(user1FundContract, tradeToken, quoteToken, sideSell, orderLimit, price1, quantity); // sell 6 VTT
    await dex.placeOrder(user2FundContract, tradeToken, quoteToken, sideBuy, orderLimit, price2, quantity); // buy 6 VTT
    await dex.placeOrder(user1FundContract, tradeToken, quoteToken, sideSell, orderFillOrKill, price2, quantity1); // sell 2 VTT
    ...
    assert.equal(viteValue1_after, new Big(initViteAmount).add(dex.getSellerObtainAmount(price2, quantity1, takerPlatformFeeRate, takerBrokerFeeRate, tradeTokenDecimal, quoteTokenDecimal)).toFixed());
    assert.equal(viteValueLocked1_after, "0");
    assert.equal(vttValue1_after, new Big(initVttAmount).sub(new Big(quantity)).sub(new Big(quantity1)).toFixed());
    assert.equal(vttValueLocked1_after, quantity);
    assert.equal(viteValue2_after, (new Big(initViteAmount).sub(dex.getBuyerCostAmount(price2, quantity1, takerPlatformFeeRate, takerBrokerFeeRate, tradeTokenDecimal, quoteTokenDecimal))
      .sub(dex.getBuyerCostAmount(price2, new Big(quantity).sub(new Big(quantity1)).toFixed(), takerPlatformFeeRate, takerBrokerFeeRate, tradeTokenDecimal, quoteTokenDecimal))).toFixed());
    assert.equal(viteValueLocked2_after, dex.getBuyerCostAmount(price2, new Big(quantity).sub(new Big(quantity1)).toFixed(), takerPlatformFeeRate, takerBrokerFeeRate, tradeTokenDecimal, quoteTokenDecimal).toFixed());
    assert.equal(vttValue2_after, new Big(initVttAmount).add(new Big(quantity1)).toFixed());
    assert.equal(vttValueLocked2_after, "0");

    await dex.cancelAllOrders(provider, tradeToken, quoteToken, user1Address, user1TradeContract, user2TradeContract);
  });

  it("test FundContract placeOrder - FillOrKillOrder - fully fill - Buy", async () => {
    const initViteAmount = "100000000000000000000000"; // 10w VITE
    const initVttAmount = "100000000000000000000000";  // 10w VTT
    ...
    await dex.placeOrder(user1FundContract, tradeToken, quoteToken, sideSell, orderLimit, price1, quantity); // sell 6 VTT
    await dex.placeOrder(user2FundContract, tradeToken, quoteToken, sideBuy, orderLimit, price2, quantity); // buy 6 VTT
    await dex.placeOrder(user2FundContract, tradeToken, quoteToken, sideBuy, orderFillOrKill, price1, quantity1); // buy 2 VTT
    ...
    assert.equal(viteValue1_after, new Big(initViteAmount).add(dex.getSellerObtainAmount(price1, quantity1, takerPlatformFeeRate, takerBrokerFeeRate, tradeTokenDecimal, quoteTokenDecimal)).toFixed());
    assert.equal(viteValueLocked1_after, "0");
    assert.equal(vttValue1_after, new Big(initVttAmount).sub(new Big(quantity)).toFixed());
    assert.equal(vttValueLocked1_after, new Big(quantity).sub(new Big(quantity1)).toFixed());
    assert.equal(viteValue2_after, (new Big(initViteAmount).sub(dex.getBuyerCostAmount(price1, quantity1, takerPlatformFeeRate, takerBrokerFeeRate, tradeTokenDecimal, quoteTokenDecimal))
      .sub(dex.getBuyerCostAmount(price2, quantity, takerPlatformFeeRate, takerBrokerFeeRate, tradeTokenDecimal, quoteTokenDecimal))).toFixed());
    assert.equal(viteValueLocked2_after, dex.getBuyerCostAmount(price2, quantity, takerPlatformFeeRate, takerBrokerFeeRate, tradeTokenDecimal, quoteTokenDecimal).toFixed());
    assert.equal(vttValue2_after, new Big(initVttAmount).add(new Big(quantity1)).toFixed());
    assert.equal(vttValueLocked2_after, "0");
